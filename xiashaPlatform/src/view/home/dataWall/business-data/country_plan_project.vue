...
<script>
	import add from "./add_data/add_country_plan_project.vue"
	...
	mixins:[commonData],
		methods: {
			getlist(){
				this.$api.getNationalProgram(this.$store.state.version,this.$store.state.year,this.$store.state.month).then(res=>{
						if(res.data!=null){
