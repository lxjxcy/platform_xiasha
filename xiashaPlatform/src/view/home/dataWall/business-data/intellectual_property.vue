...
<script>
	...
	import add from "./add_data/add_intellectual_property.vue"
	export default {
	...
	mixins:[commonData],
		methods: {
			getlist(){
				this.$api.getCompanynInnovation(this.$store.state.version,this.$store.state.year,this.$store.state.month).then(res=>{
						if(res.data!=null){
